...
import com.mx.ganks.utils.MathUtils;
...
public class PinchImageView extends ImageView {
    ...
    //外层变换的矩阵，如果是单位矩阵，那么图片是 fit center状态
    private Matrix mOuterMatrix = new Matrix();

    //手势状态：值为 PINCH_MODE_FREE ,PINCH_MODE_SCROLL,PINCH_MODE_SCALE
    ...
    //缩放模式下的缩放比例，为外层缩放值/开始缩放时的两指的距离
    private float mScaleBase = 0;


    //矩阵动画，缩放模式把图片的位置大小超出限制之后触发；双击图片放大或者缩小时触发
    ...
    public void setOnClickListener(OnClickListener onClickListener) {
        //默认的click会在任何点击情况下都会发生，所有搞成自己的
        mOnClickListener = onClickListener;
    }

    @Override
    public void setOnLongClickListener(OnLongClickListener onLongClickListener) {
        //默认的long lick会在任何点击情况下都会发生，所有搞成自己的
        mOnLongClickListener = onLongClickListener;
    }

    //不允许设置scaleType，只有内部设置的matrix
    @Override
    ...
    //获取图片最大可放大的比例,如果大于这个比例，则不被允许
    private float getMaxScale() {
        return MAX_SCALE;
    }

    //计算双击之后应该被缩放的比例，如果大于getMaxScale,
    //或者是小于 fit center的尺寸，则取边界值
    ...
    private void doubleTap(float x, float y) {
        ...
        //计算还需缩放的倍数
        animEnd.postScale(nextScale / currentScale, maxScale / currentScale, x, y);
        //将放大点移动到控件中心
    ...
    private void fling(float velocityX, float velocityY) {
        ...
        if (!(mScaleAnimator != null && mScaleAnimator.isRunning()) && getDrawable() != null) {
            ...
            mFlingAnimator = new FlingAnimator(new float[]{velocityX / 1000 * 16, velocityY / 1000 * 16});
    ...
    //当外层矩阵变换时触发
    private void onOuterMatrixChanged() {
        //用于加载超大的图片
    ...
    //获取内部的矩阵，换了图片之后，如果图片的大小不一样，会重新计算个新的从而保证 fit center的状态
    //返回的copy的值
